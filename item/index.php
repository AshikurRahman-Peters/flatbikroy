 ...
 */
$query = "SELECT COUNT(*) as num FROM $tbl_name where title!='' && status='published' && type='$type'";
$total_pages = mysql_fetch_array(mysql_query($query));
$total_pages = $total_pages['num'];
?>
<html>
	<head>
		<title>Jamibikroy.com | <?php echo $type;?></title>
							...
							<td align="left" valign="top">
								...
								<table border="0px" width="740px" style="border-collapse:collapse;">
									<tr height="40px" bgcolor="green">
										<td><font style="font-family:tahoma;font-size:14px;color:white">&nbsp;&nbsp;&nbsp;<?php echo "$type- $total_pages";?> Records found</font></td>
									</tr>
...
/* Get data. */
$sql = "SELECT * FROM $tbl_name where title!='' && status='published' && type='$type' order by adid desc LIMIT $start, $limit";
...
if ($lastpage > 1) {
	...
	//previous button
	if ($page > 1)
		$pagination .= "<a href=\"$targetpage?page=$prev&&type=$type\">< Previous</a>";
	else
	...
	//next button
	if ($page < $counter - 1)
		$pagination .= "<a href=\"$targetpage?page=$next&&type=$type\">Next ></a>";
	else
...
<?=$pagination ?>
									...
									<tr>
										<td>
											...
											if($type=="Land" || $type=="Plot" || $type=="Plot_for_Developers")
											$unit="katha";
											if($type=="Flat" || $type=="Shop")
											$unit="sft.";
											...
											?>
												<tr id="<?php echo "$row[id]";?>" onmouseover="ItemHover('<?php echo "$row[id]";?>')" onmouseout="ItemHoverOut('<?php echo "$row[id]";?>')" onclick="window.location.href='detail/?adid=<?php echo "$row[adid]";?>';" height="80px" style="border:1px solid lightgrey;cursor:pointer">
													...
													<td width="400px" align="left" valign="top">
														...
														<font style="font-family:tahoma;font-size:13px;color:gray;"><?php echo "$row[size] $unit $type";?> in <?php echo "$row[location]";?></font>
													</td>
													<td width="15px"></td>
													<td>
